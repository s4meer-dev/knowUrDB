...
export const HistoryItem: React.FC<HistoryItemProps> = ({ item, onClick }) => {
  ...
  return (
    <div 
      onClick={onClick}
      className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 hover:border-indigo-300 hover:shadow-md transition-all cursor-pointer flex flex-col sm:flex-row sm:items-center justify-between gap-4"
    >
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className="font-medium text-gray-800 truncate" title={item.question}>
            {item.question}
          </h3>
        </div>
      ...
      <div className="flex items-center text-indigo-600 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
