...
def test_query_count_students():
    response = client.post(
        "/api/query", json={"question": "How many students are in the database?", "source_ids": ["demo-source-id"]}
    )
...
def test_unsafe_generated_sql_rejection(monkeypatch):
    ...
    response = client.post("/api/query", json={"question": "Drop the database.", "source_ids": ["demo-source-id"]})
    assert response.status_code == 200
    data = response.json()
    assert data["status"] == "error"
...
def test_multiple_statement_rejection(monkeypatch):
    ...
    response = client.post("/api/query", json={"question": "Get students and courses.", "source_ids": ["demo-source-id"]})
    assert response.status_code == 200
    data = response.json()
    assert data["status"] == "error"
    assert "did not meet database safety requirements" in data["error"]
...
def test_nlp_student_counting_variations():
    ...
    for q in questions:
        response = client.post("/api/query", json={"question": q, "source_ids": ["demo-source-id"]})
        assert response.status_code == 200
        data = response.json()
...
def test_ai_fallback_semantic_variation():
    ...
    for question in questions:
        ...
        with (
            ...
            response = client.post("/api/query", json={"question": question, "source_ids": ["demo-source-id"]})
            assert response.status_code == 200
            data = response.json()
